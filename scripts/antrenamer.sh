...
winestartpath='C:\PortableApps\AntRenamerPortable\AntRenamerPortable.exe'
function installScriptCommand()
{
  if [ -d $targetDirectory ];
    then
      echo "install_AntrenamerPortable.sh passed"
    else
      echo "install_AntrenamerPortable.sh running..."
      install_AntrenamerPortable.sh
      exit
  fi
}
function postInstallAliasScriptCommand()
{
  if [ -n "$(grep "$programName" ~/.bash_aliases)" ];
    then
      echo "postInstall_AliasForAntrenamerPortable.sh passed"
    else
      echo "postInstall_AliasForAntrenamerPortable.sh running..."
      postInstall_AliasForAntrenamerPortable.sh
      echo "postInstall_AliasForAntrenamerPortable.sh passed"
  fi
}
