...
Return to [index](https://github.com/marchandd/term_ssh_wine_antrenamer/ "Index")
...
- Run one same `antrenamer.sh` command in cyclic mode to run sub programs:  
One run to set Wine emulator, one run to install and one run to launch.  
- antrenamer alias is not available at the end of progam. You have to make alias activation later.
...
- :computer: `antrenamer.sh` to set Wine emulator.  
- :computer: `antrenamer.sh` to install programm.  
- :computer: `antrenamer.sh` to launch programm.  
